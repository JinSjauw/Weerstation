...
public class Period {
    ...
    public ArrayList<Double> modus(ArrayList<Measurement> measurements){
        //ArrayList<Measurement> measurements = getMeasurements();

        int maxCount = 0;
        ArrayList<Double> modae = new ArrayList<>();

        for (Measurement forEachMeas : measurements){
            int count = 0;

            for (Measurement countingEachMeas : measurements){
                if(forEachMeas.getOutsideTemp() == countingEachMeas.getOutsideTemp()){
                    ++count;
                }
                if (count > maxCount){
                    maxCount = count;
                    ...
                    modae.clear();
                    modae.add(countingEachMeas.getOutsideTemp());

                } else if (count == maxCount){
                    if (!modae.contains(countingEachMeas.getOutsideTemp())){
                        modae.add(countingEachMeas.getOutsideTemp());
                    }
                }
            }
